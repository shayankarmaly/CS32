...
int subsequence(const Sequence& seq1, const Sequence& seq2)
{
    ...
    for (int j = 0; j < seq1.size(); j++) // checking if the size is less than the current position at seq1 is less than 0
    {
        if((seq1.size() - seq2.size() - j )< 0)
            break;
        ...
        if (firstVal == secondVal) // first condition to check to eventually go on to see if they match 
        {
            for (int z = 0; z < seq2.size(); z++)
            {
                seq1.get(j+z, firstVal);
                seq2.get(z, secondVal);

                if(firstVal!=secondVal)
                    break;

                
                else if((seq2.size()-1) == z)
                    return j;
    ...
    return -1; // if it reaches here, return -1 
}
    ...
    void interleave(const Sequence& seq1, const Sequence& seq2, Sequence& result)
    {
        ...
        Sequence tempo;
        int position = 0;
        int total = seq1.size() + seq2.size();

        for (int k = 0; k < total; k++)
        {
            if(seq1.get(k,value))
            {
                tempo.insert(position, value);
                position++;
            }
