...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    for(int unsorted = 1; unsorted < sze; unsorted++)
    {
        int location = unsorted;
        while((location > 0) && comp(s[location], s[location - 1]))
        {
            swap(s[location], s[location-1]);
            location--;
        }
    }
}
...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
    ...
    vector<Sensor*> auxSensorPointers;
    
    for (int i = 0; i != auxSensors.size(); i++)
    {
        auxSensorPointers.push_back(&auxSensors[i]);
    }
    ...
    sort(auxSensorPointers.begin(), auxSensorPointers.end(), compareSensorPtr);
